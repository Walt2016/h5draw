...
(function(root, factory) {
    ...
    function() {
        ...
        _color.prototype = {
            ...
            //#fff
            isHex3: function(color) {
                return /^#([0-9a-fA-f]{3})$/.test(color);
            },
            //#ffffff
            isHex6: function(color) {
            ...
            rgbaArr: function(rgba) {
                return rgba.match(/0\.\d{1,2}|\d{1,3}/g).map(function(t) {
                    return +t;
                })
            },
            ...
            dark: function(color, level) {
                ...
                level = level || 0.5;
                return this.rgbaWrapper(this.rgbaArr(color).map(function(t) {
                    return t * (1 - level) << 0
                }))
            },
            deepdark: function() {
            ...
            rgb: function(color, alpha) {
                ...
                if (this.isRgba(color)) {
                    if (alpha) {
                        var _rgba = this.rgbaArr(color)
                        _rgba[3] = alpha;
                        return this.rgbaWrapper(_rgba)
                    }
                    return color;
                }
                ...
                if (color.length === 4) { //#fff
                    _rgb = color.match(/[0-9a-fA-f]{1}/g).map(function(t) {
                        return '0x' + t + t << 0
                    })
                } else { //#ffffff
                    _rgb = color.match(/[0-9a-fA-f]{2}/g).map(function(t) {
                        return '0x' + t << 0
                    })
                }
                if (alpha) _rgb[_rgb.length] = alpha;
                return this.rgbaWrapper(_rgb)
            },
            //混合
            mix: function(color1, color2) {
                var color1 = this.rgb(color1),
                    color2 = this.rgb(color2),
                    a1 = this.rgbaArr(color1),
                    a2 = this.rgbaArr(color2);
                var _mix = a1.map(function(t, i) {
                    if (i === 3) {
                        if (_.isUndefined(a2[i])) {
                            a2[i] = 1;
                        }
                        return ((t + a2[i]) / 2).toFixed(2);
                    } else {
                        return (t + a2[i]) / 2 << 0;
                    }
                })
            ...
            hex: function(color) {
                ...
                if (!this.isRgba(color)) return color;
                if (this.isHex6(color)) return color;
                if (this.isHex3(color)) {
                    return color.replace(/[0-9a-fA-f]/g, function(m) {
                        return m + m;
                    });
                }
                //[r,g,b,a]
                return "#" + this.rgbaArr(color).map(function(t, i) {
            ...
            circle: function(len) {
                if (!len) {
                    len = this.len || 50
                    this.a += 360 / len
                    var r = _.cos(this.a) * 127 + 128 << 0,
                        g = _.cos(this.a + 120) * 127 + 128 << 0,
                        b = _.cos(this.a + 240) * 127 + 128 << 0;
                    return this.rgbaWrapper([r, g, b])
                }
                var arr = [];
                for (var i = 0; i < len; i++) {
                    this.len = len
                    arr[arr.length] = this.circle()
                }
                return arr;
            ...
            tween: function(color1, color2, step, easing) {
                ...
                var start = this.rgbaArr(color1),
                    end = this.rgbaArr(color2),
                    colorArr = [];
        ...
        _shapeGroup.prototype = {
            ...
            init: function(draw, opt) {
                ...
                switch (colorful) {
                    ...
                    case "gradient":
                        //渐变色
                        colorArr = co.tween(co.light(), co.dark(), len);
                        break;
                    case "singleGradient":
                        //单色渐变
                        colorArr = co.tween(co.dark(), "#ffffff", len);
                        break;
                    case "random": //随机
                        for (var i = 0; i < len; i++) colorArr[colorArr.length] = co.hex()
                        break;
                    case "solid": //单色
                        break;
                    default:
            ...
            color: function(opt) {
                ...
                var opt2 = {};
                if (colorful) opt2.color = fill ? _.color().rgba() : _.color().rgb();
                return opt2;
            },
            setup: function(opt) {
                var opt = opt || this.opt;
            ...
            surround: function(opt) {
                ...
                (function _surround() {
                    ...
                    self.shape(opt.shape);
                    //显示半径
                    opt.group.showRadius && self.draw.link([{ x: x, y: y }, { x: opt.shape.x, y: opt.shape.y }]);
        ...
        //轨迹 点图
        var _track = _.track = function(draw, opt) {
            return new _track.prototype.init(draw, opt);
        }
        _track.prototype = {
            constructor: _track,
            init: function(draw, opt) {
            ...
            polygon: function(opt) {
                ...
                function _polygon(o) {
                    self.point({ x: o.x, y: o.y, r: 1, color: color })
                }
                vs.forEach(function(t, i) {
                    var t1 = vs[i + 1 == len ? 0 : i + 1];
                    var d = t.dist(t1);
                    ...
                    ps.forEach(function(o) {
                        animate ? self.delay(function() { _polygon(o) }, time) : _polygon(o);
                    })
                });
            },
            //圆路径动画 
            circle: function(opt) {
                ...
                function _circle() {
                    p = p.rotate(1);
                    self.point({ x: p.x, y: p.y, r: 1, color: color })
                }
                var time = opt.delay || 10;
                var animate = opt.animate || opt.delay;
                for (var i = 0; i < 360; i++) {
            ...
            spiro: function(opt) {
                ...
                function _spiro() {
                    ...
                    self.point({ x: p1.x, y: p1.y, r: 1, color: color })
                }
                for (var i = 0; i < 360 * turns; i++) {
        ...
        _draw.prototype = {
            ...
            fullscreen: function() {
                return this.size({ ratio: "fullscreen" });
            },
            ...
            starrySky: function(opt) {
                ...
                function update() {
                    shadow ? self.shadow(background) : self.clear();
                    var ls = [];
                    var len = ps.length;
                    ps.forEach(function(t, i) {
                        t.move()
                        self.point({ x: t.x, y: t.y, r: t.r, color: t.color })
                    ...
                    ls.forEach(function(t) {
                        ...
                        self.setStrokeStyle(ctx, color.mix(t.from.color, t.to.color))
                        // 'rgba(200,200,200,' + alptha + ')';
                        self.link([t.from, t.to]);
                    })
                }
            ...
            firework: function(opt) {
                ...
                var Fire = function() {
                    this.x = Math.random() * range / 2 - range / 4 + center.x;
                    ...
                    this.far = Math.random() * range + (center.y - range);
                    this.base = {
                        x: this.x,
                        y: this.y,
                        vx: this.vx
                ...
                for (var i = 0; i < num; i++) {
                    listFire[listFire.length] = new Fire();
                }
                var _color = _.color()

                function update() {
                    listFire.forEach(function(t) {
                        if (t.move().reborn) {
                            var color = _color.rgb();
                            for (var i = 0; i < num * 5; i++) {
                                listFirework[listFirework.length] = new Firework(t, color);
                            }
                            t.reset()
                        }
                    })
                    ...
                    for (var i = listFirework.length - 1; i >= 0; i--) {
                        var fw = listFirework[i];
                        fw && fw.move().die && listFirework.splice(i, 1);
                    }
                }
                ...
                function render() {
                    ...
                    listFirework.forEach(function(t) {
